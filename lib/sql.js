...
module.exports = {
    ...
    QUERY_USER: (data) => {
        if (data.user_id) return `SELECT * FROM user WHERE(user_id='${data.user_id}')`
        else return `SELECT * FROM USER WHERE(user_name='${data.user_name}')`
    },
    QUERY_CATEGORY: (data) => `SELECT * FROM category WHERE(cate_id='${data.cate_id}')`,
    QUERY_PRODUCT: (data) => `SELECT * FROM product WHERE(pro_id='${data.pro_id}')`,
    QUERY_PRODUCTORDER: (data) => `SELECT * FROM productorder WHERE(order_id='${data.order_id}')`,
    ...
    INSERT_PRODUCT: (data) => `INSERT INTO product(cate_id, user_id, pro_title, pro_subtitle, pro_detail, pro_img, original_price, current_price, pro_keep, pro_code, pro_isbuy, review_status) VALUES (${data.cate_id}, ${data.user_id}, '${data.pro_title}', '${data.pro_subtitle}', '${data.pro_detail}', '${data.pro_img}', '${data.original_price}', '${data.current_price}', '${data.pro_keep}', '${data.pro_code}', ${data.pro_isbuy}, '${data.review_status}');`,
    INSERT_PRODUCTORDER: (data) => `INSERT INTO productorder(create_date, trade_date, pro_price, pro_id, buy_id, sell_id, trade_status) VALUES ('${data.create_date}', '${data.trade_date}', '${data.pro_price}', ${data.pro_id}, ${data.buy_id}, ${data.sell_id}, ${data.trade_status});`,
    ...
    /* sql 特殊sql */
    // 根据分类ID查询商品
    QUERY_PRODUCT_BY_CATEGORY: (data) => `SELECT * FROM product WHERE(cate_id=${data.cate_id})`,
    // 根据订单userID查询订单
    QUERY_PRODUCTORDER_BY_USER: (data) => `SELECT * FROM productorder WHERE(buy_id=${data.buy_id})`,
    // 根据userID查询商品
    QUERY_PRODUCT_BY_USER: (data) => `SELECT * FROM product WHERE(user_id=${data.user_id})`
}